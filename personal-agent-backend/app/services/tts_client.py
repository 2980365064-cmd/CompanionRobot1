...
resampled to 16kHz) instead of the 旧版 text2audio endpoint.
"""
...
async def tts_fetch_pcm(text: str, config: TTSConfig) -> bytes:
    ...
    Automatically chooses voice-clone or 旧版 endpoint based on config.
    Returns raw 16-bit mono 16kHz PCM — no WAV header.
    Raises TTSException on API error.
    """
    if config.use_clone:
        return await _tts_clone(text, config)
    return await _tts_旧版(text, config)


async def _tts_旧版(text: str, config: TTSConfig) -> bytes:
    """旧版 text2audio 接口。"""

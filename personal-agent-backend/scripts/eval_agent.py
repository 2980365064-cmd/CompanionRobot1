...
验证 工作上下文（会话内短期记忆）、近期记忆（跨会话工作记忆归档）、
长期记忆（长期记忆检索召回）三层记忆是否按预期协同工作。

测试场景：
  E1. 工作上下文 同一会话内上下文 —— 用户说了"下周二去上海出差"，
      紧接着问"下周二去哪？"，机器人应能正确引用刚提到的信息
  E2. 近期记忆/长期记忆 跨会话长期记忆 —— 结束会话后开启新会话，
      问"还记得我下周二去哪来着？"，应能从归档记忆中召回
  E3. 近期记忆 事实归档 —— 在会话中表达忌口偏好（不吃香菜），
      结束后开启新会话再问，应能正确回忆该事实

测试方法：
  - 通过 handle_chat 发送消息并获取回复
  - 通过 handle_session_end 结束会话（触发 工作上下文→近期记忆 归档）
...
async def run_eval() -> None:
    ...
    print("=== Eval: 工作上下文 same-session context ===")
    ...
    # 紧接着追问，验证 工作上下文 短期记忆是否保留了"上海"和"出差"
    r2, sid, _ = await handle_chat(device_id, sid, "我下周二去哪？")
    print("A2:", r2)
    ok_working_context = any(k in r2 for k in ("上海", "出差"))
    print("工作上下文 pass:" if ok_working_context else "工作上下文 fail:", ok_working_context)

    # ---------- E2: 近期记忆/长期记忆 跨会话长期记忆 ----------
    # 测试结束会话并开启新会话后，之前的信息是否被归档并可召回
    print("\n=== Eval: 近期记忆/长期记忆 cross-session ===")
    # 结束当前会话，触发 工作上下文→近期记忆 归档流程
    ...
    # 新会话中询问忌口，验证 近期记忆 是否归档了"香菜"偏好
    r5, _, _ = await handle_chat(device_id, sid3, "我有什么忌口？")
    print("A5:", r5)
    ok_fact = "香菜" in r5
    print("近期记忆 pass:" if ok_fact else "近期记忆 fail:", ok_fact)

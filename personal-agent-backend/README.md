...
cp .env.example .env   # 填入 LLM_API_KEY、EMBED_API_KEY、ES_URL
...
| **L1** | 30 轮滑动窗口；满 30 轮压缩最老 12 轮到 L2 |
| **L2** | 7 天摘要，每轮召回；过期自动汇总到 L3 |
| **L3** | Elasticsearch（BM25+向量）混合检索 + 重排序；**意图门控** |
